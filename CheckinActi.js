...
if (_currentViewId === "EndScreen") {
  ...
  if (json != null) {
    ...
    for (let x in _state.inputs) {
      if (
        _state.inputs[x].pageId === "checkin.html" &&
        _state.inputs[x].viewId === "emotions"
      ) {
      ...
      if (
        _state.inputs[x].pageId === "checkin.html" &&
        _state.inputs[x].viewId === "writing"
      ) {
        actilist[x].value = "";
        // console.log(actilist[x])
      }
    ...
    if (x.exist == true) {
      ...
      let l =
        '<input type=checkbox class="btn-check" id=' +
          ...
          isChecked +
          ">" +
          '<label class="btn btn-outline-primary me-2 buttonsMargin" for=' +
          id +
          ">" +
          x.html || x.text + "</label>";
